...
function readToml(filePath) {
  try {
    return parseTomlBasic(fs.readFileSync(filePath, "utf-8"));
  } catch {
    return null;
  }
}
 ...
 */
function readGitRemoteUrl(repoPath) {
  try {
    const gitDir = resolveGitDir(repoPath);
    if (!gitDir) return null;
 ...
 */
function identifyNode(nodePath) {
  const name = path.basename(nodePath);

  // CNR node: has .tracking file + pyproject.toml with project.name
  if (fs.existsSync(path.join(nodePath, ".tracking"))) {
  ...
  if (fs.existsSync(path.join(nodePath, ".git"))) {
    const commit = readGitHead(nodePath);
    const url = readGitRemoteUrl(nodePath);
...
async function scanCustomNodes(installPath) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      nodes.push({ ...identifyNode(nodePath), enabled: true });
    }
  } catch {}
  ...
  try {
    if (fs.existsSync(disabledDir)) {
      ...
      for (const entry of entries) {
        ...
        nodes.push({ ...identifyNode(nodePath), enabled: false });
      }
    }
  } catch {}
...
module.exports = { scanCustomNodes, identifyNode, readGitHead, readGitRemoteUrl, parseTomlBasic };
